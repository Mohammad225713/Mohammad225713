...
int main(int argc, char *argv[])
{
    QCoreApplication a(argc, argv);
        ...
        db.setDatabaseName("..\\111\\movieDB.sqlite");
   ...
   /*
    ...
    CREATE TABLE User(login VARCHAR(20) NOT NULL,
...
*/
    ...
    query.prepare("SELECT * FROM IMDB");
       if (!query.exec()) {
   ...
   */
    query.exec("SELECT title FROM IMDB JOIN  genre ON IMDB.Movie_id = genre.Movie_id where genre = 'Comedy' AND RATING >4");


    // вывести на экран
    QSqlRecord rec = query.record();
    const int loginIndex = rec.indexOf("genre");//номер "столбца"
    const int passwordIndex = rec.indexOf("title");
